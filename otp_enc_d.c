...
char process = '1'; //Encrypting = 1, Decrypting = 0
        ...
        if (numConnected < 5) { //Checks if server is full
            ...
            if (pid == 0) {
                ...
                if (receivedBuffer[0] == process) {
                    memset(ciphertextBuffer, '\0', sizeof(plaintextBuffer));
                    ciphertextBuffer[0] = '?';
                    charsRead = send(establishedConnectionFD, ciphertextBuffer, sizeof(ciphertextBuffer), 0);
                ...
                for (i = 2; i < divider; i++) {
                    plaintextBuffer[plaintextIndex] = receivedBuffer[i];
                    plaintextIndex++;
                }
                ...
                for(i = 0; i < (divider - 1); i++) {
                    if (plaintextBuffer[i] == ' ') { plaintextBuffer[i] = '@'; }
                    if (keyBuffer[i] == ' ') { keyBuffer[i] = '@'; }

                    plainNum = (int) plaintextBuffer[i];
                    keyNum = (int) keyBuffer[i];

                    plainNum -= 64;
                    keyNum -=64;

                    cipherNum = ((keyNum + plainNum) % 27);
                    ...
                    ciphertextBuffer[i] = temp;
                }

                ciphertextBuffer[i-1] = '\n';

                charsWritten = send(establishedConnectionFD, ciphertextBuffer, sizeof(ciphertextBuffer), 0);
                if (charsRead < 0) { error("ERROR writing to socket."); }
